...
#### Step 4: 完了画面
- **目的**: オンボーディング完了を祝う演出画面
- **演出**: 画面表示時に紙吹雪が自動で降る（1.2秒）
- **UI**: 完了メッセージ + 「マイページへ」ボタン
- **操作**: [マイページへ] ボタンで遷移

### 共通機能

- **プログレスインジケーター**: 1-2-3-4 の丸数字でステップ表示
...
### 概要
オンボーディングの最終ステップ（Step 4）へ遷移した時点で、紙吹雪/パーティクル効果による完了演出を自動表示します。

### 仕様
- **演出タイプ**: 画面両端から中央に向かって紙吹雪が降る
- **継続時間**: 1.2秒
- **カラー**: Tailwind blue系（`#3b82f6`, `#60a5fa`, `#93c5fd`）
- **トリガー**: Step 4 への画面遷移時（自動）、またはグローバルスキップ時
- **アクセシビリティ**: OSの「視差効果を減らす」設定に対応（`prefers-reduced-motion`）

### 実装ライブラリ
- `canvas-confetti` v1.9.4（軽量、gzip後約3KB）

### UI挙動

#### 通常フロー（Step 1-2-3-4）
1. ユーザーが Step 3 で「次へ」をクリック
2. Step 4（完了画面）へ遷移
3. 画面表示と同時に紙吹雪演出が自動開始
4. 「マイページへ」ボタンをクリックで `/mypage` へ遷移

#### グローバルスキップ
1. ユーザーが「すべてスキップする」をクリック
2. 紙吹雪演出を開始
3. 演出中はボタンを無効化
4. 演出終了後（1.2秒後）、自動的に `/mypage` へ遷移
